...
def process_clustering(data,image_path):
    ...
    data = data.drop(columns=['decorated_indicator','Unnamed: 0','masked_item_impression','masked_shop_page_view',
    ...
    kmean_score = metrics.silhouette_score(X, labels, metric='euclidean')
    ...
    plt.xlabel('Shops')
    plt.ylabel('Euclidean distances')
    plt.savefig(image_path+'clustering_agglo_single.png')
    ...
    print(X2['agglomerative.complete.labels'].value_counts())
    print(X2['agglomerative.single.labels'].value_counts())
